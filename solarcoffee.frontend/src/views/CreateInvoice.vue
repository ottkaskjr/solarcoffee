...
import { IProductInventory } from '@/types/IProductInventory';
...
export default class CreateInvoice extends Vue {
  ...
  get canGoNext() {
    // if on step 1 you can go next if selectedCustomerId != 0
    if (this.invoiceStep === 1) {
      return this.selectedCustomerId !== 0; //true/false
    }
  ...
  async initialize(): Promise<void> {
    //customerService.getCustomers().then(res => this.customers = res).catch() OR
